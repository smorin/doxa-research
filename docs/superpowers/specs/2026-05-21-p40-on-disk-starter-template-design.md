...
- **New `doxa config validate [PATH]` subcommand** in `cli_subcommands/config.py`. With no PATH, validates `~/.config/doxa/doxa.config.toml`. With PATH, validates that file against `DoxaConfig`. When PATH resolves to the shipped starter template, also runs the bidirectional drift check. Supports `--json`.
...
- **Bidirectional drift test** (`tests/test_starter_template_matches_schema.py`):
  - Schema → template: every `StarterField` in `DoxaConfig` exists at the expected TOML path with the expected default value.
  - Template → schema: `DoxaConfig.model_validate(tomllib.loads(template_text))` succeeds (catches stray keys, typos, `extra="forbid"` violations).
...
|---|---|---|
| Schema correctness (types, required, `extra="forbid"`) | `DoxaConfig` Pydantic models | `model_validate()` at runtime + tests |
| "Which fields ship to users on `init`" | `StarterField(...)` marker in schema | Walked by drift test |
| Default *values* for shipped fields | `starter.config.toml` (on-disk) | Drift test pins each value against the schema marker's default |
| Comment prose users see | Inline in `starter.config.toml` | Reviewer eyeball; no test |
| Validation pipeline | `doxa config validate` reads TOML → `model_validate` → optional drift check if PATH resolves to shipped template | Same code path as the drift test |
...
| `src/doxa_research/cli_subcommands/_config_validate.py` | Pure validation function `validate_config_file(path: Path, *, drift_check: bool) -> ValidationResult`. |
...
| `tests/test_lefthook_starter_hook.py` | Smoke test for the lefthook hook (TS05). |
...
| `tests/test_init_ships_profiles.py` | If it imports `STARTER_PROFILES` directly, switch to parsing the template; otherwise leave alone. |

### Deleted files

| Path | Why |
|---|---|
| `src/doxa_research/_starter_data.py` | `STARTER_PROFILES` moves inline into the template's `[profiles.*]` blocks. Only `commands.py` imports it. |
...
doxa config validate [PATH] [--json]
  ...
  └─► _config_validate.validate_config_file(target_path, drift_check=is_shipped_template)
        ...
        │
        ├─► DoxaConfig.model_validate(doc)             # schema phase
...
|---|---|---|---|---|
| Template missing from package | `importlib.resources` raises `FileNotFoundError` (broken wheel) | 1 | `package data missing: doxa_research/data/starter.config.toml` | `{ok: false, error: "package_data_missing", path: …}` |
| PATH does not exist | User typo, deleted file | 1 | `no such file: <path>` | `{ok: false, error: "file_not_found", path: …}` |
| TOML parse error | Syntax error | 1 | `TOML parse error at line N: <msg>` — surface `tomllib` message verbatim | `{ok: false, error: "toml_parse_error", path: "<file>:<line>", message: …}` |
| Schema validation error | `model_validate` raises `ValidationError` | 1 | `<dotted.toml.path>: <pydantic_msg>` — first error in the brief; full list in `--json` | `{ok: false, error: "schema_validation", errors: [{path: …, message: …, input: …}]}` |
| Drift: missing `StarterField` key | Template missing a path the schema marks `in_starter=True` | 1 | `starter template missing required key: <path> (expected default: <repr>)` | `{ok: false, error: "drift_missing_key", path: …, expected: …}` |
| Drift: value mismatch | Template has the key but value ≠ schema default | 1 | `starter template value mismatch at <path>: expected <repr>, got <repr>` | `{ok: false, error: "drift_value_mismatch", path: …, expected: …, actual: …}` |

### Success

| Case | Exit | Stdout (human) | Stdout (`--json`) |
|---|---|---|---|
| Validation + drift check both passed | 0 | `OK: <path> (schema + drift)` | `{ok: true, path: …, checks: ["schema", "drift"]}` |
| Validation passed, drift check skipped | 0 | `OK: <path> (schema)` | `{ok: true, path: …, checks: ["schema"]}` |

### Behavior rules

- **Fail-fast on parse errors.** If TOML doesn't parse, do not attempt `model_validate`.
- **Fail-fast within schema phase.** Report the first `ValidationError`'s first error; full list in `--json`.
...
- **Errors → stderr; JSON envelope → stdout.** Matches existing `config get/set/list` convention.
...
| ID | File | Asserts |
|---|---|---|
| TS01 | `tests/test_starter_template_matches_schema.py` | Bidirectional drift. Schema → template: every `StarterField` in `DoxaConfig` exists at the expected TOML path with the expected default value. Template → schema: `DoxaConfig.model_validate(tomllib.loads(template_text))` succeeds. |
...
| TS04b | (same file) | Mutated value in a tempfile copy → exit 1, stderr names the offending path. |
| TS04c | (same file) | Missing `StarterField` key in a tempfile copy → exit 1, stderr names the missing path. |
| TS04d | (same file) | `--json` envelope shape on success and failure paths (verify `ok`, `path`, `error`, `expected`, `actual` keys). |
...
| TS05 | `tests/test_lefthook_starter_hook.py` | Pre-commit hook smoke test. Create a temp git repo, copy `lefthook.yml` + a corrupted `starter.config.toml`, run `lefthook run pre-commit`, assert non-zero exit and that stderr contains a drift-mismatch line. |
| (existing) | `tests/test_init_ships_profiles.py` | Update only if it imports `STARTER_PROFILES` directly. |
...
5. **TS04a–f fail** → add `_config_validate.py` + `@config.command("validate")` wiring + `docs/COMMANDS.md` row → TS04 passes.
6. **TS05 fails** → add `lefthook.yml` entry → TS05 passes.
...
- `doxa config validate <path>` exits 1 with a precise error path when any `StarterField` default in the schema differs from the template's value at the same TOML path.
- `doxa config validate ~/.config/doxa/doxa.config.toml` (no PATH form) validates the user-tier config against `DoxaConfig` and reports any schema violation.
...
- **Backward compat.** None broken externally. The starter file format and contents from a user's perspective gain a `[clarification]` section (visible additive change). Internal API consumers of `_starter_data.STARTER_PROFILES` would break — `git grep` confirms only `commands.py` imports it.
...
- **LOC delta:** roughly −250 (remove generator + `_starter_data.py` + `WRITER_COMMENTS`) and +220 (template file with `[clarification]`, drift test, new `doxa config validate` subcommand + `_config_validate` module, lefthook entry, hook smoke test). Net simplification ~30 lines.
...
- Whether TS05 (lefthook smoke test in a temp git repo) is worth its complexity, or whether the hook should be hand-verified once and trusted thereafter.
